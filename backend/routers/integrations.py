...
async def get_google_data(
    ...
    try:
        if google_token.expires_at and google_token.expires_at < datetime.now():
            if google_token.refresh_token:
                ...
                raise HTTPException(
                    status_code=401, 
                    detail="Token expired and no refresh token available. Please reconnect your Google account."
                )
    except Exception as e:
        logging.error(f"Error refreshing token: {str(e)}")
        raise HTTPException(
            status_code=401,
            detail="Failed to refresh access token. Please reconnect your Google account."
        )
    ...
    try:
        if service == "gmail":
            ...
            try:
                data = google_service.get_sheets_files(google_token.access_token)
                if not data:
                    raise HTTPException(
                        status_code=404,
                        detail="No Google Sheets files found or insufficient permissions"
                    )
            except Exception as sheets_error:
                logging.error(f"Sheets error: {str(sheets_error)}")
                raise HTTPException(
                    status_code=500,
                    detail=f"Failed to fetch Sheets data: {str(sheets_error)}"
                )
        elif service == "slides":
            data = google_service.get_slides_files(google_token.access_token)
        elif service == "forms":
            data = google_service.get_forms_files(google_token.access_token)
        else:
        ...
        logging.error(f"Failed to fetch {service} data: {str(e)}")
        raise HTTPException(
            status_code=500,
...
async def sync_all_integrations(
    ...
    if google_token:
        try:
            ...
            sheets = google_service.get_sheets_files(google_token.access_token, max_results=5)
            slides = google_service.get_slides_files(google_token.access_token, max_results=5)
            forms = google_service.get_forms_files(google_token.access_token, max_results=5)
